<x-analytics::root area="admin" class="an:space-y-6">
    ...
    <div class="an:flex an:flex-wrap an:items-start an:justify-between an:gap-4">
        ...
        <x-ui::button variant="secondary" x-on:click="$anOpenWhenDone($wire.$refs.adForm.$wire.editAd({{ $detail->id }}), 'an-ad-form')"><x-ui::icon name="pencil-square" class="an:h-4 an:w-4" /> {{ __('Modifier la pub') }}</x-ui::button>
    </div>
