...
it('resolves the most specific ad whose conditions all match the session params', function () {
    ...
    expect($builder->resolveAd(['src' => 'meta_ete', 'creative' => 'cabrio'])->id)->toBe($cabrio->id)  // 2 conditions win over 1
        ->and($builder->resolveAd(['src' => 'meta_ete', 'creative' => 'other'])->id)->toBe($generic->id)
        ->and($builder->resolveAd(['src' => 'meta_ete'])->id)->toBe($generic->id);
});
...
it('returns null when a condition is unmet, and resolves the campaign independently', function () {
    ...
    expect($builder->resolveAd(['src' => 'meta_hiver', 'creative' => 'cabrio']))->toBeNull() // src differs
        ->and($builder->resolveAd(['src' => 'meta_ete']))->toBeNull()                        // creative missing
        ->and($builder->resolveAd([]))->toBeNull()
        ->and($builder->resolveCampaign(['src' => 'meta_ete'])->id)->toBe($ete->id)
        ->and($builder->resolveCampaign(['src' => 'meta_hiver']))->toBeNull();
});
...
it('ignores inactive ads and conditionless definitions', function () {
    ...
    expect((new MarketingReportBuilder)->resolveAd(['src' => 'meta_ete']))->toBeNull();
});
...
it('aggregates ad-driven traffic per campaign and ad over the period', function () {
    ...
    expect($builder->headline($period, null))->toBe(['sessions' => 3, 'visitors' => 2]);
