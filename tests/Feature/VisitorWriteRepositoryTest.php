...
it('creates a visitor with its identity', function () {
    $now = CarbonImmutable::parse('2026-06-30 10:00:00');

    $visitor = $this->repo->create('uuid-1', $now, ['type' => 'client', 'id' => 7]);

    expect($visitor->uuid)->toBe('uuid-1')
        ->and($visitor->first_seen_at->toDateTimeString())->toBe('2026-06-30 10:00:00')
        ->and($visitor->last_seen_at->toDateTimeString())->toBe('2026-06-30 10:00:00')
        ->and($visitor->session_count)->toBe(0)
        ->and($visitor->subject_type)->toBe('client')
        ->and($visitor->subject_id)->toBe(7);
});

it('creates an anonymous visitor without a subject', function () {
    $visitor = $this->repo->create('uuid-2', CarbonImmutable::now(), null);
...
it('refreshes last seen and stitches the subject once', function () {
    $visitor = $this->repo->create('uuid-3', CarbonImmutable::parse('2026-06-01 00:00:00'), null);
...
it('never overwrites an already stitched subject', function () {
    $visitor = $this->repo->create('uuid-4', CarbonImmutable::now(), ['type' => 'client', 'id' => 1]);
...
it('increments the session count', function () {
    $visitor = $this->repo->create('uuid-5', CarbonImmutable::now(), null);
